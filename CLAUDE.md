...
- **bot owns the promoter**: `promote_due_actions`, `release_stale_claims`, `expire_stale_management` all run inside `bot.py` as asyncio tasks (see `post_init`), not as separate cron jobs. The bot process is always running when the user expects signals, so co-location avoids a 4th service.
...
A per-channel self-distillation loop that lowers interpreter cost over time WITHOUT losing accuracy. The expensive interpreter's verdict is ground-truth: every interpreted message is captured to `learning_samples` with its `category`/`action_types` and the triage decision (`orchestrator.process_message`, additive hook — never affects the trade path). A rolling sweeper in the bot process (`learning_loop`) waits until a channel accumulates `learning_batch_n` new samples, then embeds them (OpenAI `text-embedding-3-small`, reusing `trigger_matcher._embed_batch`), greedily clusters by category + cosine, synthesizes one human-readable rule per dense cluster (cheap triage model), and **replays each candidate rule over the whole corpus** to attach evidence — crucially `false_suppression_count`, the number of real signals a suppression rule would have hidden.

The loop is **propose-only**: it writes ranked suggestions to `learning_suggestions`; nothing activates automatically. The operator reviews them in the Suggestions GUI tab. A suggestion is one-tap-acceptable only when provably safe (suppression rules: `false_suppression_count == 0` and enough support; action triggers: purity ≥ threshold and no conflicts); unsafe ones require an explicit confirm. Accept is the ONLY mutation of `profile.json` (`profile_writer`, atomic), routing each rule into the SAME fields the live layers already read (`noise_patterns`, `triage_keep_triggers`, `triggers[]`), so it takes effect via their mtime caches with no restart. This complements `cost_guard` (the hard daily-budget brake): the CLL lowers the baseline so the brake is rarely hit.

Tuning lives in settings (`learning_enabled`, `learning_batch_n`, `learning_corpus_max`, `learning_min_cluster_size`, `learning_embed_threshold`, `learning_min_support`, `learning_min_purity`, `learning_suggestion_ttl_days`). Scope note: v1 ships noise-suppression, action-trigger, and triage keep/context-drop suggestions; continuous profile-vocabulary refinement and shadow re-evaluation (drift detection) are planned v2 fast-follows. See `docs/superpowers/specs/2026-05-29-channel-learning-loop-design.md`.
...
- `src/promoter.py` — lifecycle sweepers: `promote_due_actions` (pending→sent when `execute_after` elapses), `release_stale_claims` (recovers claims a crashed EA never confirmed), and `expire_stale_management` (fails ticketless singleton-management actions that lingered past `MANAGEMENT_MAX_AGE_SEC`). **Safety contract for ticketless management types** (`SINGLETON_MANAGEMENT_TYPES`: MOVE_SL_BE/MOVE_SL/CLOSE_PARTIAL/CLOSE_FULL/REOPEN_LAST/REINFORCE/TIGHTEN_SL/MODIFY_TPS): because they resolve their target via the EA's `FindSingletonOpenTicket` at execution time, a stale one must never fire — `release_stale_claims` marks them `failed`/`stale_claim_not_retried` instead of recycling to `sent`, and `expire_stale_management` fails any that sat in `sent` too long. This prevents the diagnostics-SMC-2026-05-29 failure where a resurrected CLOSE_FULL (meant for an already-SL'd position) closed a freshly opened trade. OPEN/legacy-ticketed types keep recycling (duplicate fire blocked by the `api.py` `claim_expired` guard).
...
- `src/api.py` — FastAPI app. `GET /positions?status=open` is the EA's reconciliation oracle. `GET /positions/last_closed` and `POST/GET /market/price` are the Phase-1 endpoints driving the SYSTEM STATE prompt block. `POST /actions/{id}/resize_pending` sets an explicit `pending_lot_override` + monotonic `resize_seq` on a `watching` pending OPEN (operator override that bypasses the EA's risk cap); the EA applies it on its next `/actions/{id}` poll by delete+replace. The `/market/price` heartbeat now also persists `account_balance`/`account_equity`/`account_at` (settings keys) so the GUI's resize warning can express risk as a % of balance (STALE after 60s → falls back to absolute dollars). A `watching`→`watching` re-POST (the EA re-confirming the new broker ticket after a resize) updates `ea_response` without re-stamping `executed_at` or re-firing the terminal notification.
- `src/llm_provider.py` — provider abstraction. `LLMProvider` Protocol + `AnthropicProvider` and `OpenAIProvider` concretes. `AI_PROVIDER` env switch picks one. Usage dict normalized to `{input_tokens, output_tokens, cache_read_tokens, cache_creation_tokens}` so `logs/ai_calls.jsonl` is provider-agnostic. `LLMProvider.interpret()` accepts an optional `image_bytes: bytes | None` parameter; both `AnthropicProvider` and `OpenAIProvider` prepend an image content block when provided (vision pass for the image fallback).
- `src/learning_store.py` — labeled corpus for the Channel Learning Loop. Captures every interpreted message + category/action_types + triage decision, keyed by `source_channel_id`. Dedups per channel on stripped raw text. Capture swallows all errors (live-path safety). Supersedes `unmatched_store`.
- `src/channel_learner.py` — pure batch learner: greedy cosine clustering by category + per-cluster LLM synthesis + replay evidence (incl. `false_suppression_count`, `purity`). No DB/network; deps injected.
- `src/suggestion_store.py` — proposed cheap-layer rules (`proposed|accepted|dismissed|expired`), dedup via a partial UNIQUE index on (channel, dedupe_key) WHERE status='proposed', plus TTL expiry.
- `src/profile_writer.py` — the sole writer of accepted rules into `profile.json` (atomic temp+os.replace). Maps rule_kind → existing profile field (noise_patterns / triage_keep_triggers / triggers[]).
- `src/bot_loops/learning_loop.py` — rolling per-channel scheduler; after `learning_batch_n` new samples it embeds, learns, and writes suggestions. Fully wrapped so a learner failure can't touch the trade path.
- `src/gui/views/suggestions_view.py` + `suggestions_logic.py` — operator review tab; Accept routes a suggestion into `profile.json` via `profile_writer` (the only activation step).
- `ea/CopyTrades.mq5` + `ea/Dashboard.mqh` — MQL5 EA. Magic number `919191`. Canvas dashboard (left-anchored, 380×900px, hash-gated repaint). `g_plans[]` persisted to MT5 `GlobalVariables` so restart doesn't lose in-flight zone watches. `ExecuteOne` dispatcher routes 11 action types (12 types minus `ALERT` which only the bot consumes). `HeartbeatMarketPrice()` POSTs bid/ask every `MarketPriceHeartbeatSec` (default 15s). `ManagePendingOrders` also honors `pending_lot_override`/`resize_seq` from the action payload: it feasibility-checks margin (deferring if `OrderCalcMargin` isn't ready yet), deletes the broker pending order, and re-places at the new lot via the shared `PlacePendingLimit` helper — keeping the same action row (only the broker ticket swaps).
