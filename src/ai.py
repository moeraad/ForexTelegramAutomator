...
Stale-parent exception for CANCEL_PENDING: if the message body itself contains a clear cancellation/deletion phrase (e.g. "delete", "cancel", "close pending", "close this order", "cancel order", "delete limit", "delete order") AND PENDING OPEN SIGNALS shows at least one [watching] OPEN — emit CANCEL_PENDING even when the reply parent no longer exists in SYSTEM STATE. The watching OPEN is the live target; the parent's expiry is irrelevant. Do NOT emit ALERT in this case.
If the parent points at a position/order that no longer exists in SYSTEM STATE AND there is no independent basis for action (no watching OPEN for a cancellation request, no open position for a management request), emit ALERT level="warning" text="[partial] reply target not in state; manual review".
